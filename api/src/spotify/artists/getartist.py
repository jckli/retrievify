...
async def get_artist(request, id):
    ...
    if access_token is None:
        return await get_artist_refresh(spotify, body.get("refresh_token"), id)
    user_resp = await spotify.get_artist(access_token, id)
...
async def get_artist_refresh(spotify, refresh_token, id):
    no_access = response.json({"status": 401, "message": "No access"})
