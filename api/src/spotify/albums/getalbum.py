...
async def get_albums(request, id):
    ...
    if access_token is None:
        return await get_album_refresh(spotify, body.get("refresh_token"), id)
    user_resp = await spotify.get_album(access_token, id)
...
async def get_album_refresh(spotify, refresh_token, id):
    no_access = response.json({"status": 401, "message": "No access"})
