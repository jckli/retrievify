  ...
  $.get("/ajax/top_artists?type=" + selection, function (data) {
    ...
    for (var i = 0; i < artists.length; i++) {
      ...
      $(artists[i])
        ...
        .find(".name-link")
        .find(".spotify-link")
        .attr("href", data[i].external_urls.spotify);
      $(artists[i])
        .find(".artist-info")
        .find("#artist-genre")
        .text(data[i].genres[0]);
    }
  });
});
      ...
      $.get("/ajax/currently_playing?type=now", function (data) {
        if (data == null) {
          ...
          if (oldname == null && oldimage == null) {
            ...
            $(".song-now-playing").append(
              '<div class="song-info song-info-np"></div>'
            );
            $(".song-info-np").append(`<div class="title-with-link"></div>`);
            $(".title-with-link").append(
              `<h2 id="song-title">${song_name}</h2>`
            );
            $(".title-with-link").append(
              `<a href="${link}" class="spotify-link"><i class="fa-brands fa-spotify fa-lg"></i></a>`
            );
